...
**Trigger (user):** "Start a Docs-Only Architecture Finalization Gate before continuing schema migration, realtime, or feature implementation… so future implementation cannot accidentally build against the old `tenant → channel → conversation` model." Originating strategy draft: `docs/main-stratergy.md`.
