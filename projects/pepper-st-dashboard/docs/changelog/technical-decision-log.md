...
## 2026-06-17 — Chat Monitor conversation boundary: read-only verification + Contact-Thread model (ADR-0016) (DOCS/SCHEMA-PROPOSAL ONLY; no code, no migration, no DB write, no `ai.*`, no commit/push)

### TD-091 — Adopted the **contact-thread** conversation boundary (ADR-0016); proposed `app_conversation_sessions`; revised ADR-0003/0012, refined ADR-0015
**Trigger (user):** "Proceed with the next gate: ADR-0016 + Architecture Docs + Schema Proposal Gate … Dashboard conversation = customer/contact thread; Agno session = provider/internal session segment; one dashboard conversation may contain many Agno/provider sessions."
- **Decision (contract, not implemented):** the dashboard **conversation = a customer/contact thread** (`app_conversations`, **one row per contact**); each Agno session is an internal **provider session** (`app_conversation_sessions`). **Supersedes `1 Agno session = 1 dashboard conversation`.** Boundary: **`tenant_id + business_id + channel_id + external_contact_id`** (transitional, pre-business: `tenant_id + channel_id + external_contact_id`). Authoritative: **ADR-0016** + `docs/architecture/09`.
- **Proposed schema (document only):** `dashboard.app_conversation_sessions (id, tenant_id, business_id NULLABLE, conversation_id, provider default 'agno', external_session_id, started_at, last_at, created_at, updated_at)` with **`unique(tenant_id, provider, external_session_id)`**, `conversation_id → app_conversations`, **no FK to `ai.*`**, `external_session_id == ai.agno_sessions.session_id` **by value**. `app_conversations` becomes the contact thread; **`agno_session_id` moves off** it. Revises **ADR-0012** (4-table count → +1; by-value-contact/read-only principles kept) and **ADR-0003** (grain → contact thread); refines **ADR-0015** (conversation node = Contact Thread → Provider Sessions → Agno Session).
- **Migration (propose only):** **expand** (add `app_conversation_sessions`, keep existing temporarily) → **backfill** (one link row per conversation: `external_session_id` = old `agno_session_id`; then **collapse** rows sharing the boundary key into one thread per contact) → **verify** (no `ai.*` change; every session linked once; threads == distinct contacts; message counts preserved; no raw PII) → **enforce** (drop `agno_session_id` from `app_conversations`; add contact-thread uniqueness; keep provider-session uniqueness). Non-destructive; `ai.*` untouched.
- **Read-path (propose only):** list **one row per contact thread** (preview/`last_at` across all linked sessions); transcript **loads all linked sessions**, parses, excludes system/tool/internal/`from_history`, **dedupes by a stable provider message id**, sorts by `created_at`/`ts`, renders one continuous thread. The positional `safeMessageId(conversationId, index)` is **not stable after merge** → replace with an **opaque id derived from the stable provider message id** (never the raw id).
- **Realtime (extends ADR-0014/0015):** events **target the contact thread** (`conversation_id`); `external_contact_id` + `provider_session_id` are **server-side only**; a new provider session for an existing contact is a **thread update, never a new row**. Transport stays **SSE**. PII deny-list extended with `external_session_id` / `provider_session_id`.
- **Docs:** **created** `docs/adr/0016-contact-thread-conversation-boundary.md`; **updated** `docs/architecture/09`, `docs/architecture/08`, `CONTEXT.md`, `docs/phases/phase-1-post-acceptance-hardening.md`, `docs/v2/06-demo-readiness-and-next-steps.md`, this log; **revise-banners** on `docs/adr/0003`, `docs/adr/0012`, `docs/architecture/03-agno-mapping.md`, `docs/architecture/01-data-model.md`, `docs/database/03-dashboard-data-contract.md`.
- **Scope/safety:** **documentation + schema proposal only.** No product code, no migration, no DB write, no `ai.*` change, no commit/push. Implementation is **approval-gated**.

### TD-090 — Read-only verification PROVED the boundary defect (one Agno session treated as one conversation → duplicate Chat Monitor rows)
**Trigger (user):** "Read-Only Verification Gate for the Chat Monitor conversation boundary issue." Method: existing **read-only** verifiers (`db:agno:verify`, `db:agno:reconfirm` [pins `default_transaction_read_only=on`], `db:chat:verify`); all output **masked**.
- **DB facts (masked, live `papper`):** `app_conversations` = **19 rows for 16 distinct contacts** (≥3 duplicate rows); visible Chat Monitor list = **6 rows** with masked contact **`94•••••273` appearing twice** (duplicate row **proven** on live data); **`session_id`** = 32–36 chars, not phone-like → opaque **provider session id**; **`user_id`** = 11–22 chars, 0 nulls → the **contact**; run roles assistant 220 / user 124 / system 124 / tool 105 (parser excludes system+tool). 
- **Code facts:** `app_conversations` keyed `(tenant_id, channel_id, agno_session_id)` (`lib/db/schema.ts`); sync upserts **one row per session** (`lib/agno/sync.ts`); transcript loads **ONE session** (`lib/chat-monitor/service.ts`); parser carries a **stable Agno `m.id`**, excludes system/tool/`from_history` (`lib/agno/parser.ts`); wire ids are **opaque** (`safeMessageId`, `maskContactId`) — **no raw PII leak** in list/transcript payloads.
- **Conclusion:** duplicate rows **do** happen; the transcript **does** miss the contact's other-session messages; **root cause** = the per-session grain (ADR-0003/0012). Fix proposed in **ADR-0016** (TD-091).
- **Scope/safety:** **read-only.** No DB writes, no migrations, no `ai.*` changes, no commits.

## 2026-06-16 — Architecture Finalization Gate: multi-business / branch / channel / conversation model (DOCS-ONLY; no code, no migration, no `ai.*`, no commit/push)

### TD-089 — Locked the dashboard hierarchy as `Tenant → Business → optional Location → Channel → Conversation → Agno Session` (`tenant ≠ business`); ADR-0015 + architecture/09
**Trigger (user):** "Start a Docs-Only Architecture Finalization Gate before continuing schema migration, realtime, or feature implementation… so future implementation cannot accidentally build against the old `tenant → channel → conversation` model." *(The originating strategy draft `main-stratergy.md` was **later deleted as stale** — superseded by `docs/architecture/09` + ADR-0016.)*
- **Decision (contract, not yet implemented):** the dashboard is **`Tenant → Business → optional Location → Channel → Conversation → Agno Session`** — a **tenant is the SaaS/billing/owner boundary**, a **business** is a brand/line inside it (a tenant has **one default** business at onboarding but **may have many**), a **location** is an optional branch (0..N; `location_id = NULL` = not-applicable/not-known-yet), a **channel** is one real external account with **`type` separate from `external_channel_id`** (never store the platform name as the provider id), and every **conversation owns `tenant_id`+`business_id`+`channel_id`** (optional `location_id`). **Branch-aware routing is a premium capability.** Authoritative docs: **ADR-0015** + **`docs/architecture/09-multi-business-branch-channel-strategy.md`** (full rules, visual tree, **Mermaid ERD**, incoming-message resolution, migration plan).
- **Schema direction:** target = **7 core tables** — `app_tenants`, `app_businesses` (NEW), `app_locations` (NEW), `app_channels` (+`business_id`/`location_id`/`type`/`external_channel_id`), `app_conversations` (+`business_id`/`location_id`/`location_source`/`location_confidence`), `app_ai_agent_bindings` (NEW), `app_realtime_outbox` (NEW); plus existing `app_tenant_entitlements`. Optional later: `app_users` + 3 access tables (only when staff permissions ship). **This supersedes the "exactly 4 tables" count of ADR-0012** but **keeps ADR-0012's by-value-contact + no dashboard customer/identity table + grain-lock** (one Agno `session_id` = one conversation).
- **Agno boundary (reaffirmed):** `ai.*` **read-only**; mapping **by value** (`app_conversations.agno_session_id = ai.agno_sessions.session_id`); **no FK from `dashboard.*` to `ai.*`**; the Agno `agent_id` shape is **not hard-coded** — it resolves through `app_ai_agent_bindings.external_agent_id` to tenant/business/location/channel.
- **Realtime (extends ADR-0014):** transport **stays SSE**; the **in-process polling detector stays current**, Agno **webhook is future-preferred**; events become **scope-aware** (`tenant/business/location/channel/conversation`) + carry **safe deltas/patches** (no whole-API refetch per message); a durable **`app_realtime_outbox`** is the target backing for the in-memory bus. **No WebSocket; no `LISTEN/NOTIFY` on `ai.*`; no Redis/queue.** **No raw phone/`user_id`/`external_contact_id`/`agno_session_id`/`runs`/`session_data`** in any payload.
- **Migration (when approved):** **expand → backfill → verify → enforce** — add tables + **nullable** `business_id`/`location_id`; backfill a **default business** under the existing PEPPER ST. tenant (map existing channels/conversations to it, `location_id = NULL` unless known); verify counts + `ai.*` untouched + nothing deleted; only then enforce NOT-NULL + dashboard-only FKs/indexes. **Destructive change forbidden.**
- **Docs touched (this gate):** **created** `docs/adr/0015-…` + `docs/architecture/09-…`; **updated** `docs/architecture/08` §5 (scope-aware realtime), `docs/phases/phase-1-post-acceptance-hardening.md` (gate + paused 12F note), `docs/v2/06-demo-readiness-and-next-steps.md` (next steps), `CONTEXT.md` (hierarchy + new glossary terms), and old-model banners on `docs/architecture/01-data-model.md` + `docs/architecture/04-multitenancy.md`.
- **Scope/safety:** **documentation only.** No product code, no migration, no `ai.*`/`dashboard.*` write, no commit/push. Implementation (schema/onboarding/realtime-scope/UI-filters) is **approval-gated** and **must not begin** until this contract is approved.

## 2026-06-16 — Slice 12F realtime FOUNDATIONS (ADR-0014): safe event contract + in-memory bus + SSE transport (then PAUSED for the architecture gate)

### TD-088 — Decided realtime detection/transport (ADR-0014) and built 12F-1/12F-2; paused before the detector for the ADR-0015 scope gate
**Context:** product mandate (TD-081) that realtime is required (no manual `db:agno:sync` during customer use). Architecture approved by the user: **in-process polling detector + SSE browser transport; Agno webhook = future-preferred; no WebSocket; no DB trigger/`LISTEN/NOTIFY` on `ai.*`; no Redis/queue; no PII in payloads; no global loader/flicker.**
- **Decision:** recorded as **ADR-0014** (`docs/adr/0014-realtime-detection-and-sse-transport.md`).
- **12F-1 (built, TDD):** `lib/realtime/events.ts` — `RealtimeEvent` union (`conversation.created|updated`, `transcript.updated`, `metrics.updated`, `coverage.updated`) with a **strict per-type key whitelist + deep PII deny-list** (`phone`/`user_id`/`external_contact_id`/`session_id`/`agno_session_id`/`runs`/`session_data`), `assertSafeRealtimeEvent`/`serializeRealtimeEvent`/`toSseData`/`parseRealtimeMessage`; `lib/realtime/bus.ts` — in-memory pub/sub (subscriber isolation, accurate count, process singleton). 
- **12F-2 (built):** `app/api/events/stream/route.ts` — long-lived **SSE** endpoint (subscribe → safe frames + heartbeat, teardown on disconnect, `text/event-stream`, `no-store`, `x-accel-buffering:no`); `components/shell/use-realtime.ts` — `EventSource` client hook that re-validates every frame.
- **Proof:** realtime suite **24/24**, `typecheck` clean; dev SSE proof on `:3411` (`200 text/event-stream`, `EventSource` OPEN, preamble `retry: 3000\n\n: connected\n\n`, no console errors).
- **PAUSED:** the detector + single-flight lock + UI wiring (12F-3…7) are **not built**. Per **TD-089 / ADR-0015**, the event contract will be **extended scope-aware** (tenant/business/location/channel + safe deltas) **before** resuming.
- **Safety:** SSE payloads carry only a safe type + the internal conversation UUID; **no** raw phone/`user_id`/`external_contact_id`/`agno_session_id`/`runs`/`session_data`. **No** `ai.*`/`dashboard.*` writes, **no** DB/migration. *(No commit/push.)*

## 2026-06-16 — Slice TIME-CONSISTENCY: one tenant-timezone, AM/PM time formatter shared by Dashboard, Chat Monitor & Analytics (UI-only; no DB, no migration, no `ai.*`/`dashboard.*` writes, no commit/push)

### TD-087 — Unified every human-facing timestamp on the tenant timezone (Asia/Colombo) in 12-hour AM/PM; removed Chat Monitor's hardcoded-UTC rendering
**Trigger (user):** the same moment displayed differently across the app — "dashboard shows one time, chats another, the system another; the AM/PM time must sync." Example: the Chat Monitor header read `16 Jun 2026, 15:11 UTC` while the Dashboard's Last activity for the same instant read `Jun 16, 20:41`.
- **Root cause (PROVEN, not assumed):** there was **no single source of truth** for time formatting. Chat Monitor (`components/chat-monitor/chat-monitor.tsx`) formatted **everything in UTC** via `getUTC*` (the helpers `fmtFull`/`fmtClock`/`fmtDayLabel`/`fmtListStamp`, deliberately UTC "to avoid hydration drift"), the Dashboard (`lib/dashboard/presenter.ts` `fmtDateTime`) formatted in the **tenant timezone but 24-hour** (`hourCycle:"h23"`), and Analytics used the tenant timezone for date-only labels. So one instant rendered as UTC-AM/PM in chat, tenant-24h on the dashboard, and with a stray `UTC` suffix in the chat header — three visibly different strings.
- **Fix (single shared formatter; smallest correct change):** new pure `lib/format/time.ts` — `fmtClock` / `fmtDateTime` / `fmtFullStamp` / `fmtDayLabel` / `fmtListStamp` / `dayKey`, all formatting in an **explicit `timeZone`** via `Intl.DateTimeFormat` (12-hour AM/PM). An explicit zone makes output depend only on the instant + the fixed tenant zone (never the host's local zone), so it is **deterministic on server and client** — the old "render in UTC" hydration workaround is no longer needed. `DEFAULT_TIME_ZONE` ("Asia/Colombo") is now defined here and **re-exported** from `lib/analytics/ranges.ts` (one source of truth). Dashboard `fmtDateTime` now delegates here (24h → AM/PM). Chat Monitor's local UTC helpers were **deleted**; the tenant `timeZone` is added to `ConversationListPayload` (populated from `tenant.timezone` in `getConversationList`) and threaded through `ConversationRow`/`ConversationDetail`/`MessageBubble`.
- **Tests (TDD, fail-first):** new `lib/format/time.test.ts` (15) — AM/PM + noon/midnight boundaries, `timeZone` honored (UTC vs Asia/Colombo differ), null/invalid sentinels (`—` / `""` / `Unknown`), `dayKey` zone mapping, Today/Yesterday/long-date labels, list-stamp cases, and a **cross-surface consistency** test asserting `fmtClock` and `fmtDateTime` agree on the clock for one instant. `lib/dashboard/presenter.test.ts` updated 24h→AM/PM (`Jun 15, 7:00 PM`). Full suite **250/250**, `typecheck` clean.
- **Browser proof (dev `:3411`, desktop 1920) — BEFORE/AFTER:** **Before** = chat header `… 15:11 UTC`, dashboard `… 20:41` (different). **After:** Dashboard `Last activity = Jun 16, 8:41 PM`, Timezone `Asia/Colombo`; Chat Monitor header `last seen 16 Jun 2026, 8:41 PM` — **same instant, same zone, same AM/PM** (8:41 PM Colombo = the old 15:11 UTC); transcript bubbles AM/PM in Colombo; Analytics tz label `Asia/Colombo`. `hasUTC=false` on every page; **no console errors / no hydration mismatch**. (Note: the chat header's `last seen 8:41 PM` vs the latest bubble `11:52 PM` is **stale `last_at`**, not a formatting issue — the transcript reads Agno live while the list metadata is from the last sync; this is the data-freshness concern handled by Slice 12F.)
- **Safety / scope:** UI/formatting only. No new dependency, no API shape change beyond the additive `timeZone` string on the list payload, no DB/migration, no `ai.*`/`dashboard.*` writes, no PII added (a tenant IANA zone is not PII).
...
### TD-083 — Fixed WhatsApp-like scroll-anchor preservation on "load older" (the reading position no longer jumps)
**Trigger:** with pagination counts correct (TD-082), the real UX bug remained: prepending older messages **jumped** the view to the older top instead of holding the reading position.
- **Root cause:** `ConversationDetail`'s scroll layout-effect was keyed on `[messages, ready]` and consumed the saved geometry (`pendingPrepend`) on the **`loadingOlder` spinner render** — `loadOlder()` first sets `loadingOlder:true` (a new `ready` object, but the SAME `messages` array ref), which fired the effect and consumed the anchor **before** any messages were prepended; the real prepend then had nothing to restore → jump.
- **Fix (element anchor, measurement-based):** new pure `components/chat-monitor/scroll-anchor.ts` `reanchorScrollTop(currentScrollTop, currentOffset, desiredOffset)` (+ `scroll-anchor.test.ts`, 5). Each bubble now carries `data-mid={message.id}`. On scroll-up, `captureTopAnchor` records the message nearest the viewport top + its offset BEFORE the load; the layout-effect — now keyed on **`[messages]` only**, so the spinner toggle can't consume it — finds that same element AFTER the prepend lands and scrolls so it returns to its captured offset. Measurement-based ⇒ immune to the spinner/"Load older" button/day-separator heights and the browser's own scroll anchoring. Initial open still jumps to the latest (bottom).
- **Browser proof (dev `:3411` AND production `:3412`, instrumented `getBoundingClientRect`):** opened Sameen (75 msgs). Mid-viewport anchor (dev): offset **234 → 233 (diff 1px)**; top anchor (prod): **0 → 0 (diff 0px)** — same message stays put, no jump to older top, no jump to bottom, while **+20** messages prepend. Pagination unchanged: **20 + 20 + 20 + 15 = 75**, one request per cursor (55→35→15), **75/75 unique (no duplicates)**, final page `hasMoreBefore:false`. `typecheck` clean, **222/222** tests, `build` green. No DB/`ai.*`/`dashboard.*`/PII change (UI-only).

### TD-084 — WhatsApp-quality scroll anchoring: stopped the browser's native scroll anchoring from fighting the manual correction (the residual ~"80%" shift)
**Trigger:** TD-083 made the *settled* anchor delta 0px, yet a slight shift "after data loads" remained in real use. Settled measurements missed it.
- **Root cause:** the scroll container (`.wa-chat-bg`) had **no `overflow-anchor`**, so it defaulted to `auto` — the **browser's own scroll anchoring re-adjusted `scrollTop` a frame after the manual `useLayoutEffect` correction**, producing a transient shift. Secondary: the "Load older messages" **button (~28px) ↔ spinner (~18px)** swap changed the top-loader height (~10px transient).
- **Fix (3 parts, all UI-only):** (1) `style={{ overflowAnchor: "none" }}` on the scroll container → only the manual element-anchor correction runs, nothing fights it; (2) the correction is now **two-pass** — synchronous in `useLayoutEffect` (no uncorrected frame painted) **plus** one `requestAnimationFrame` residual pass for variable bubble heights — **instant, never animated** (no `scrollIntoView`/smooth); (3) the loader row is **fixed height** (`h-11`) so the button↔spinner swap can't bump the anchor. New pure `anchorCorrectionDelta(before, after)` helper (+2 tests) alongside `reanchorScrollTop`.
- **Browser proof — FRAME-BY-FRAME (dev `:3411` AND prod `:3412`):** sampled the anchor's offset on **every animation frame** across the spinner→prepend→correct sequence (not just settled). Dev: **maxDeviation 1px across 24 post-prepend frames**; prod: **maxDeviation 1px across 83 frames**; the **first painted frame** after the prepend is already within 1px (no flash). Pagination clean: initial `limit=20`, `before=55/35/15`, **20+20+20+15=75**, one request per cursor, **75/75 unique (no duplicates)**. Security: responses carry no `runs`/`session_data`/`user_id`/`session_id`/`external_contact_id`/`phone`; transcript DTO keys = `{createdAt,id,role,text}`. **No console errors.** `typecheck` clean, **224/224** tests, `build` green. No DB/`ai.*`/`dashboard.*` change.

### TD-085 — The REAL older-load jump: the anchor was captured at fetch-START, not at prepend time, so scrolling during the async fetch made it stale (~70px upward snap). Also: TD-083/084 were proven on the WRONG layout.
**Trigger:** the user's screen recording still showed a clear downward/upward shift on older-message load, despite TD-083/084 claiming "≤1px". Re-investigation found **two** problems — one product bug, one test-harness lie.
- **Why the previous proof was false (honest correction):** (1) the automated browser was **901px wide** → below the `lg` (1024px) breakpoint, so the chat panel ran the **mobile single-column layout** and the desktop scroll container measured **`clientHeight: 0`** — the "1px" frames were measured on a collapsed panel, not the desktop layout the user records at 2560×1440. (2) The repro **triggered the load with `el.scrollTop = 0`** (an instant jump), which captures the anchor and lands the prepend at the *same* scroll position — it structurally **cannot** reproduce a scroll-during-fetch race. Both made a real bug look fixed.
- **Root cause (product):** `triggerLoadOlder` captured the anchor **once, at the moment the fetch was kicked off** (when `scrollTop` crossed the 72px threshold). With a real fast scroll the user keeps moving during the async fetch — typically all the way to `scrollTop 0` — so by the time the prepend committed, the captured anchor described a position the user had **already scrolled past**. The element-anchor correction faithfully restored that **stale** position, snapping the viewport up by ≈ the trigger threshold. Measured on desktop (1680×1000): the top-visible message jumped **−69px** (≈ the 72px threshold). `handleScroll` also **stopped updating while `loadingOlder`** (early-return guard), guaranteeing the anchor went stale exactly during the fetch.
- **Fix (minimal, UI-only):** keep the anchor pinned to the user's **current** top-visible message — re-capture it on **every** scroll event, **including while an older page is in flight** (new `if (ready.loadingOlder) { anchorRef.current = captureTopAnchor(el); return; }` branch). The post-prepend `useLayoutEffect` correction then restores where the user **actually is** when the DOM commits, not where the fetch began. `triggerLoadOlder` → `beginLoadOlder` (button path captures at click; the scroll path is now continuous). No change to the pure `reanchorScrollTop`/`anchorCorrectionDelta`, the two-pass correction, `overflow-anchor:none`, or the fixed-height loader (all still required). The parent `loadOlder` already had a synchronous `inFlightOlderRef` guard, so continuous capture cannot double-fire a page.
- **Browser proof — DESKTOP layout (viewport 1680×1000), realistic flick-scroll that reaches `scrollTop 0` during the fetch:**
  - **Dev `:3411`** — 3 consecutive older loads, anchor = first top-visible message each time: P1 `relativeTop 105→104` **topDelta −1px**; P2 `105→104` **−1px**; P3 `105→104` **−1px**; **maxDeviation 1px every frame**; the 4th attempt correctly **no-ops** (`hasMoreBefore:false`). **Button** path (mid-viewport anchor, `scrollTop 600`, scroll-trigger not involved): `relativeTop 51→51` **topDelta 0px**. Counts **20→40→60→75**, cursors `55/35/15`, **no duplicate messages**. **No console errors.**
